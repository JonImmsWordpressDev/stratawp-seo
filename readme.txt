...
* New: Source material. Paste up to 5 URLs and/or notes; URLs are fetched one at a time through WordPress' safe HTTP API (8 second timeout each, 25 second total budget, private and local addresses rejected on the first request and on every redirect, 1.5 MB cap, one-hour cache for successes), reduced to readable text and passed to the AI in a fenced block with instructions to base facts on it, paraphrase and cite the URLs as the external links. Fetch problems are listed in the result panel and never block generation.
